...
from exceptions import (
    BookNotAvailableError, BookNotFoundError,
    MemberNotFoundError, LoanNotFoundError
)
...
def lib():
    ...
    library.register_member("Alice", "M00001")
    library.register_member("Bob",   "M00002")
    return library
...
def test_checkout_returns_loan(lib):
    loan = lib.checkout("M00001", "001")
    assert loan is not None
    assert loan.isbn      == "001"
    assert loan.member_id == "M00001"
...
def test_checkout_unavailable_raises(lib_with_loan):
    with pytest.raises(BookNotAvailableError):
        lib_with_loan.checkout("M00002", "001")

def test_checkout_unknown_member_raises(lib):
    with pytest.raises(MemberNotFoundError):
        lib.checkout("M99999", "001")
...
def test_return_book_makes_available(lib_with_loan):
    lib_with_loan.return_book("M00001", "001")
